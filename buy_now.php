        ...
        if ($flag == 0)
            ...
            foreach ($stmt_user as $row_user)
                if ($row_user['user_amount'] >= $total) {
                    ...
                    $stmt->execute(['transaction_user_id' => $id, 'transaction_send_to' => 'Order', 'transaction_amount' => $total, 'transaction_method' => 'Ordered', 'transaction_added_by' => $id, 'transaction_type'=>1, 'transaction_date'=>$today, 'date_transaction'=>$date]);
                } else
