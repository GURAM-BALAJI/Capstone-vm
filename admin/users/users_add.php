...
if (isset($_POST['add'])) {
	...
	if ($row['numrows'] > 0) {
		...
		try {
			...
			$stmt->execute(['transaction_user_id' => $user_id, 'transaction_send_to' => 0, 'transaction_amount' => $amount, 'transaction_method' => 'login', 'transaction_added_by' => $by, 'transaction_type' => 0, 'transaction_date'=>$today, 'date_transaction'=>$date]);
